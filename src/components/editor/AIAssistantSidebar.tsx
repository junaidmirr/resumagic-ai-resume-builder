...
export function AIAssistantSidebar({
  ...
  const [activeTab, setActiveTabState] = useState<Tab>(
    aiSessionCache.activeTab,
  );
  const [jobDescription, setJobDescriptionState] = useState(
    aiSessionCache.jobDescription,
  );
  ...
  const [tabStates, setTabStates] = useState<Record<string, TabCache>>(
    aiSessionCache.tabStates,
  );
  const [followUpPrompt, setFollowUpPromptState] = useState(
    aiSessionCache.followUpPrompt,
  );
  ...
  const handleApplyFix = async (fix: AIFixItem) => {
    ...
    setLoadingAction(`applying_fix_${fix.id}`);
    try {
      await onApplyFix(fix);
      setAppliedFixIds((prev) =>
        prev.includes(fix.id) ? prev : [...prev, fix.id],
      );
    } catch (err: any) {
  ...
  const handleAction = async (action: string, text: string = "") => {
    if (!user) {
      openModal({
        ...
        subtitle: "Please log in to use AI Assistant features.",
    ...
    try {
      ...
      const res = await fetchWithCaptcha("/api/ai-assistant", {
        method: "POST",
        headers,
        body: JSON.stringify({
          action,
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        throw new Error(
          `Server returned an error (${res.status}). Please restart your local Python backend.`,
        );
      }
      ...
      if (data.status === "rejected") {
        setRejectionReason(
          data.reason ||
            (data as any).error ||
            "I am a dedicated Resume AI Assistant and can only assist with career, resume, and job application requests.",
        );
        return;
      }
      ...
      if (!res.ok)
        throw new Error(
          (data as any)?.error || `Generation failed (${res.status})`,
        );
  ...
  return (
    <div className="flex flex-col h-full bg-app-surface text-app-text">
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
        ...
        {activeTab === "import" && (
          <div className="flex flex-col gap-4 h-full">
            <div className="p-4 bg-app-bg border border-app-border rounded-xl shadow-sm hover:border-brand-primary/30 transition-colors">
              ...
              <p className="text-[10px] text-app-text-secondary mb-3 leading-relaxed">
                Upload a PDF or DOCX to automatically parse and layout your
                existing resume.
              </p>
            ...
            <div className="p-4 bg-app-bg border border-app-border rounded-xl shadow-sm hover:border-brand-secondary/30 transition-colors">
              <h4 className="text-xs font-bold text-app-text mb-2 flex items-center gap-2">
                <div className="p-1.5 bg-[#0a66c2]/10 rounded-md">
                  <div className="w-3.5 h-3.5 font-bold text-[#0a66c2] flex items-center justify-center text-[10px]">
                    in
                  </div>
                </div>
              ...
              <p className="text-[10px] text-app-text-secondary mb-3 leading-relaxed">
                Paste your public profile URL to generate a resume instantly.
              </p>
              <input
                type="text"
              ...
              <button
                ...
                className="w-full bg-[#0a66c2] text-white text-xs py-2.5 rounded-lg font-bold hover:bg-[#004182] transition-colors shadow-lg shadow-[#0a66c2]/20"
              >
                Import Profile
              </button>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <button
                ...
                className="p-3 bg-app-bg border border-app-border rounded-xl hover:border-brand-primary/50 transition-all text-left group disabled:opacity-50 disabled:cursor-not-allowed hover-lift"
              >
                {loadingAction === btn.id ? (
                  <Loader2 className="w-4 h-4 animate-spin text-brand-primary mb-2" />
                ) : (
                  <btn.icon className="w-4 h-4 text-app-text-muted group-hover:text-brand-primary mb-2 transition-colors" />
                )}
            ...
            <button
              onClick={() => handleAction("generate_projects", jobDescription)}
              disabled={loadingAction !== null}
              className="p-3 bg-app-bg border border-app-border rounded-xl hover:border-brand-accent/50 transition-all text-left group col-span-2 disabled:opacity-50 disabled:cursor-not-allowed hover-lift"
            >
              {loadingAction === "generate_projects" ? (
                <Loader2 className="w-4 h-4 animate-spin text-brand-accent mb-2" />
              ) : (
                <Sparkles className="w-4 h-4 text-app-text-muted group-hover:text-brand-accent mb-2 transition-colors" />
              )}
              <span className="text-xs font-semibold text-app-text">
                Generate Projects
              </span>
            ...
            <button
              onClick={() => handleAction("generate_cover_letter")}
              disabled={loadingAction !== null}
              className="p-3 bg-brand-primary/10 border border-brand-primary/20 rounded-xl hover:border-brand-primary/50 hover:bg-brand-primary/20 transition-all text-left group col-span-2 disabled:opacity-50 disabled:cursor-not-allowed hover-lift"
            >
              {loadingAction === "generate_cover_letter" ? (
                <Loader2 className="w-4 h-4 animate-spin text-brand-primary mb-2" />
              ) : (
                <FileText className="w-4 h-4 text-brand-primary mb-2" />
              )}
            ...
            {[
              {
                ...
                sub: "Find missing keywords & flaws",
                icon: Search,
              ...
              },
              {
                id: "match_resume",
                label: "Match to Job",
                sub: "Get a fit score & tailored advice",
                icon: Target,
              ...
              <button
                ...
                onClick={() => handleAction(btn.id)}
                disabled={
                  btn.id === "match_resume" || btn.id === "analyze_job"
                    ? !jobDescription || loadingAction !== null
                    : loadingAction !== null
                }
                className="flex items-center gap-3 p-3 bg-app-bg border border-app-border rounded-xl hover:border-brand-primary/50 transition-all text-left group disabled:opacity-50 disabled:cursor-not-allowed hover-lift"
              >
                ...
                <div>
                  <span className="text-sm font-semibold text-app-text block">
                    {btn.label}
                  </span>
        ...
        {activeTab === "chat" && (
          <div className="flex flex-col gap-3 h-full">
            <p className="text-sm text-app-text-secondary">
              Ask the AI to write a resume from scratch, outline a specific
              role, or brainstorm ideas.
            </p>
          ...
          loadingAction !== "applying_fix" && (
            <div className="flex flex-col items-center justify-center p-8 text-app-text-muted space-y-2">
              ...
              <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-brand-primary/10 border border-brand-primary/20 text-brand-primary rounded-full font-mono text-xs font-bold">
                <Clock className="w-3 h-3 animate-pulse" /> Elapsed:{" "}
                {formattedTimer}
              </div>
            </div>
          )}
        ...
        {fixes.length > 0 &&
          (loadingAction === null || loadingAction === "applying_fix") && (
            <div className="mt-4 flex flex-col gap-3">
              ...
              {fixes.map((fix) => (
                <div
                  key={fix.id}
                  className="p-3 bg-app-bg border border-app-border rounded-xl flex flex-col gap-2 shadow-sm"
                >
                  <div className="flex items-center justify-between">
                    ...
                    {onApplyFix &&
                      (appliedFixIds.includes(fix.id) ? (
                        <span className="px-2.5 py-1 bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 rounded-md text-[10px] font-bold flex items-center gap-1 cursor-default select-none shadow-xs">
                        ...
                        <button
                          ...
                          disabled={loadingAction !== null}
                          className="px-2.5 py-1 bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-600 hover:to-emerald-600 text-white rounded-md text-[10px] font-bold hover:shadow hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
                        >
                          {loadingAction === `applying_fix_${fix.id}` ? (
          ...
          <div className="mt-4 flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {copied ? (
                    ...
                    <Copy className="w-3 h-3 text-app-text-muted" />
                  )}
                  {copied ? "Copied!" : "Copy"}
                </button>
              </div>
            ...
            <div className="mt-2 flex flex-col gap-1.5 bg-app-bg p-3 rounded-xl border border-app-border">
              <label className="text-[10px] font-bold uppercase tracking-wider text-app-text-muted flex items-center gap-1">
                <Wand2 size={12} className="text-brand-primary" /> Modify with
                Follow-up
              </label>
