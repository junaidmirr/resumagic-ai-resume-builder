          ...
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden lg:flex items-center gap-6">
              <button
                ...
                className="text-sm font-medium text-app-text-secondary hover:text-app-text transition-colors"
              >
                Features
              </button>
              <button
              ...
              <Link
                ...
                className={`text-sm font-medium transition-colors ${isNavActive("/resources/interview-guide") ? "text-app-text font-semibold" : "text-app-text-secondary hover:text-app-text"}`}
              >
                Interview Guide
              </Link>
              <Link
                to="/resume-examples"
                className={`text-sm font-medium transition-colors ${isNavActive("/resume-examples") ? "text-app-text font-semibold" : "text-app-text-secondary hover:text-app-text"}`}
              >
                Examples
              </Link>
              <Link
                to="/pricing"
                className={`text-sm font-medium transition-colors ${isNavActive("/pricing") ? "text-app-text font-semibold" : "text-app-text-secondary hover:text-app-text"}`}
              >
                Pricing
              </Link>
            </nav>
            ...
            <div className="hidden md:flex items-center gap-3">
              <button
                onClick={toggleTheme}
                className="p-2 text-app-text-secondary hover:text-app-text hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors"
                title="Toggle Theme"
              >
              ...
              {user ? (
                <div className="flex items-center gap-3 relative">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2.5 px-3 py-1.5 rounded-lg border border-app-border hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors focus:outline-none"
                  >
                    <div className="w-6 h-6 rounded bg-slate-800 dark:bg-slate-200 text-white dark:text-slate-900 flex items-center justify-center font-semibold text-xs">
                      {user.email?.charAt(0).toUpperCase() || "U"}
                    </div>
                    <span className="text-xs font-medium text-app-text max-w-[130px] truncate">
                      {user.email}
                    </span>
                  </button>
                  ...
                  <AnimatePresence>
                    {isUserMenuOpen && (
                      <motion.div
                        ...
                        className="absolute right-0 mt-2 w-56 bg-app-surface rounded-lg shadow-lg border border-app-border origin-top-right z-50 p-1"
                      >
                        <div className="space-y-0.5">
                          <Link
                            to="/dashboard"
                            onClick={() => setIsUserMenuOpen(false)}
                            className="flex items-center gap-2.5 px-3 py-2 text-xs font-medium text-app-text hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md transition-colors"
                          >
                            <LayoutDashboard className="w-4 h-4 text-app-text-secondary" />
                            Dashboard
                          </Link>
                          <Link
                            to="/resources/interview-guide"
                            ...
                            className="flex items-center gap-2.5 px-3 py-2 text-xs font-medium text-app-text hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md transition-colors"
                          >
                            <Target className="w-4 h-4 text-app-text-secondary" />
                            Interview Prep Guide
                          </Link>
                          <div className="h-px bg-app-border my-1"></div>
                          <button
                            onClick={handleLogout}
                            className="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-medium text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-md transition-colors text-left"
                          >
                ...
                <>
                  <button
                    ...
                    className="text-xs font-semibold text-app-text hover:text-slate-600 dark:hover:text-slate-300 transition-colors px-3 py-2"
                  >
                    Log In
                  </button>
                  <button
                    onClick={() => openModal({ title: "Create your account" })}
                    className="px-4 py-2 rounded-lg font-semibold text-xs text-white bg-slate-900 hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white transition-colors"
                  >
                    Start Building
                  </button>
                </>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      AI Resume Builder
                      <span className="ml-auto text-[10px] bg-brand-primary/10 text-brand-primary px-2 py-0.5 rounded-full font-bold">
                        PRO
                      </span>
                    </Link>
                    <button
