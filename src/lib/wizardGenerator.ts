...
// ─── Generator ───────────────────────────────────────────────────────────────
export function generateWizardElements(
  data: WizardData,
  level: TemplateLevel = "level1",
): EditorElement[] {
  ...
  if (level === "level2") {
    ...
    const addLeftSection = (title: string) => {
      ...
      els.push({
        ...
        page_id: "page-1",
        x: 18,
        y: PAGE_HEIGHT - leftTop,
        x2: sidebarW - 18,
        y2: PAGE_HEIGHT - leftTop,
        border_color: "#2563EB",
        border_width: 1.5,
        z_index: 1,
      });
    ...
    const addLeftText = (txt: string, isMuted = false, isBold = false) => {
      ...
      els.push({
        ...
        text: txt,
        x: 18,
        y: PAGE_HEIGHT - leftTop - h,
        width: sidebarW - 36,
        height: h,
        font_size: isMuted ? 8 : 8.5,
        font_name: isBold ? "Helvetica-Bold" : FONT,
        text_color: isMuted ? "#64748B" : "#334155",
        bold: isBold,
        z_index: 1,
      });
      leftTop += h + 2;
    ...
    addLeftText(
      `${data.contact.city || ""} ${data.contact.country || ""}`.trim(),
    );
    ...
    els.push({
      ...
      element_type: "text",
      page_id: "page-1",
      text: (data.experiences[0]?.role || "PROFESSIONAL").toUpperCase(),
      x: mainX,
      y: toY(14),
      width: mainW,
      height: 14,
  ...
  if (level === "level4") {
    ...
    els.push({
      ...
      shape_type: "path",
      page_id: "page-1",
      path_d: `M ${sidebarW} ${PAGE_HEIGHT - 105} C ${sidebarW + 80} ${PAGE_HEIGHT - 90} ${sidebarW + 140} ${PAGE_HEIGHT - 120} ${PAGE_WIDTH} ${PAGE_HEIGHT - 105} L ${PAGE_WIDTH} ${PAGE_HEIGHT} L ${sidebarW} ${PAGE_HEIGHT} Z`,
      x: 0,
      y: PAGE_HEIGHT,
      fill_color: "#6366F1",
      border_width: 0,
    ...
    if (data.skills.length >= 3) {
      ...
      for (let i = 0; i < skills.length; i++) {
        ...
        els.push({
          ...
          page_id: "page-1",
          text: skills[i],
          x: centerX + (radius + 15) * Math.cos(angle) - 15,
          y: centerY + (radius + 15) * Math.sin(angle) - 5,
          width: 30,
          height: 10,
          font_size: 6.5,
          font_name: "Helvetica-Bold",
  ...
  if (validExps.length > 0) {
    ...
    for (const exp of validExps) {
      ...
      // Company + location
      if (exp.company) {
        text(exp.company + (exp.location ? `  ·  ${exp.location}` : ""), 9, {
          color: C_ACCENT,
          bold: true,
          lineHeight: 1.3,
        });
  ...
  if (hasAdditional) {
    ...
    if (certificates?.trim()) {
      text(`Certifications: ${certificates}`, 9, {
        color: C_BODY,
        lineHeight: 1.4,
      });
      top += 3;
    ...
    if (extracurriculars?.trim()) {
      text(`Activities: ${extracurriculars}`, 9, {
        color: C_BODY,
        lineHeight: 1.4,
      });
      top += 3;
